...
// var options.RedisSingleClient = require('redis'),
...
function RedisSentinelClient(options) {
  ...
  if (options.master_debug) {
    options.RedisSingleClient.debug_mode = true;
  }
...
RedisSentinelClient.prototype._connectSentinel = function (port, host) {
  ...
  */
  ...
  this.sentinelTalker = new options.RedisSingleClient.createClient(port, host);
  ...
  var sentinelListener = new options.RedisSingleClient.createClient(port, host);
...
RedisSentinelClient.prototype._connect = function (port, host) {
  var self = this
    ...
    , thisClient = new options.RedisSingleClient.createClient(port, host, self.masterOptions);
...
options.commands.forEach(function (command) {
  ...
  RedisSentinelClient.prototype[command] = function (args, callback) {
    ...
    if (Array.isArray(args) && typeof callback === "function") {
      ...
      // return sentinel.send_command(command, arguments);
    }
  };
});
...
RedisSentinelClient.prototype.SENTINEL =
  function (args, callback) {
    ...
    if (Array.isArray(args) && typeof callback === "function") {
      ...
      return sentinel.sentinelTalker.send_command('sentinel', arguments);
    }
  }
...
var Multi = options.RedisSingleClient.Multi;
...
function createCustomRedisSentinel (_options) {
  if (!(_options.RedisSingleClient && _options.RedisCommands)) {
    throw new Error('no options.RedisSingleClient or RedisCommands as parameters')
  }
  RedisSingleClient = _options.RedisSingleClient
  commands = _options.RedisCommands


  var internals = { }
  // internals.RedisSentinelClient = RedisSentinelClient;
