...
suite('sentinel failover', function(){
  ...
  before( function(done){
    ...
    this.waitForEvent = function(eventName, callback){
      ...
      if (_suite.eventLog.indexOf(eventName) > -1) {
        ...
        // wait
        _suite.sentinelClient.once(eventName, function(){
          callback()
        })
      }
    }
  ...
  test('redis servers are running as expected', function(done){
    ...
    async.series({
      master: function(ok){
        pidHelpers.findPid(['redis-server', '*:5379'], ok)
      },
      slave: function(ok){
        pidHelpers.findPid(['redis-server', '*:5380'], ok)
      },
      sentinel: function(ok){
        pidHelpers.findPid(['redis-server', '*:8379'], ok)
      }
    }, function(error, pids){
      if (error) return _suite.emitError(error)
